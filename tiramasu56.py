...
def denseBlock(t, nb_layers,growth_rate,name):
    with tf.name_scope(name) as scope:
        for i in range(nb_layers):
            tmp = t
            t = BatchNormalization(axis=3,gamma_regularizer=l2(0.0001),beta_regularizer=l2(0.0001),name=scope+'bn'+str(i))(t)
            t = Activation('relu',name=scope+'act'+str(i))(t)
        # the filter of conv means grouth rate
            t = Conv2D(growth_rate, kernel_size=(3, 3), padding='same', kernel_initializer='he_uniform',
                       data_format='channels_last',name=scope+'c'+str(i))(t)
            t = Dropout(0.2,name=scope+'d'+str(i))(t)
            t = concatenate([t, tmp])
    return t

def transitionDown(t, nb_features,name):
    with tf.name_scope(name) as scope:
        t = BatchNormalization(axis=3,gamma_regularizer=l2(0.0001),beta_regularizer=l2(0.0001),name = scope + 'bn')(t)
...
def inference(inputs):
    ...
    nb_features = 48
    #donw sample x2
    t = Conv2D(48, kernel_size=(7, 7), strides=(2,2), padding='same',
    ...
    for i in range(n_pool):
        t = denseBlock(t, layer_per_block[i],growth_rate,'DB'+str(i))
    ...
    # center
    t = denseBlock(t,layer_per_block[n_pool],growth_rate,'center') # bottle neck
    #up sampling
    ...
    for i in range(n_pool):
        ...
        t = denseBlock(t, layer_per_block[iter],growth_rate,name='DB'+str(iter))
